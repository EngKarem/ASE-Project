...
//        String area = input.next();
    ...
    @Override
    public ArrayList<PassengersRequests> viewAlRequests() {
        return db.getRequests();
    }

    @Override
    public boolean Activate(String type,int id) {
        if (type.equals("Passenger")) {
            ...
            for (Passenger user : db.getPassengers())
            {
                if(user.getID() == id)
                {
                    user.setActivateStat("Activated");
                    System.out.println(user);
                    return true;
                }
            }
            ...
            for (Driver driver : db.getDrivers())
            {
                if(driver.getID() == id)
                {
                    driver.setActivateStat("Activated");
                    return true;
                }
            }
        }
