...
public class RegisterServices implements RegisterIServices {
    ...
    public Boolean PassengerSignUp(Passenger passenger) {
        db.AddPassengers(new Passenger(passenger.getID(), passenger.getUsername(), passenger.getEmail(), passenger.getPassword(), passenger.getMobileNumber(),"User"));
        return true;
    }

    @Override
    public Boolean DriverSignUp(Driver driver) {
        db.AddDrivers(new Driver(driver.getID(), driver.getUsername(), driver.getEmail(), driver.getPassword(), driver.getMobileNumber(),"Driver", driver.getDriving_license(), driver.getNationalID()));
        return true;
    }
}
